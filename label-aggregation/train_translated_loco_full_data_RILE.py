...
def main():
    ...
    # Need to restart in the middle, skip the countries
    # that we already covered.
    with open('results_full_data_translated_vanilla_mpnet_RILE_strict_predictions.json', 'r') as inp:
        processed_manifestos = json.load(inp)
    countries_done = set()
    for key in processed_manifestos:
        countries_done.add(key.split('-')[0])
    ...
    for country_idx in tqdm(range(len(countries)), desc=f'Test countries'):
        ...
        if test_country in countries_done:
            continue
        print(test_country)
        ...
        if rile_strict:
            ...
            log_file_name = 'results_full_data_translated_vanilla_mpnet_RILE_strict.log' if sbert else 'results_full_data_translated_vanilla_roberta_RILE_strict.log'
        elif rile_only:
            rile_dict = utils.get_rile_categories()
            ...
            train_data.insert(0, 'label', new_train_labels)
            test_data.insert(0, 'label', new_test_labels)
            log_file_name = 'results_full_data_translated_vanilla_mpnet_RILE_only.log' if sbert else 'results_full_data_translated_vanilla_roberta_RILE_only.log'
        else:
            log_file_name = 'results_full_data_translated_vanilla_mpnet.log' if sbert else 'results_full_data_translated_vanilla_roberta.log'
