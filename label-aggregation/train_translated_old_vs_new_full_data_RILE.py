...
def train_epoch(
    ...
    for i in tqdm(range(epoch_train_steps), desc=f'Epoch {epoch_n+1}', leave=False):
        ...
        sentences = data['text_translated'][lo:hi].to_list()
...
def main():
    ...
    # We will be predicting the last two years, 2020 and 2021, 
    # from all of the old data.
    ...
    batch_size = 32*8 if sbert else 16*8
    # batch_size = 64*9 if sbert else 24*8
    n_epochs = 5
    ...
    train_data = all_data.loc[ all_data.year < 2019 ]
    test_data = all_data.loc[ all_data.year >= 2019 ]
    ...
    if rile_strict:
        ...
        log_file_name = 'results_full_data_translated_old_vs_new_mpnet_RILE_strict.log' # if sbert else 'results_full_data_translated_old_vs_new_roberta_RILE_strict.log'
    elif rile_only:
        rile_dict = utils.get_rile_categories()
        ...
        train_data.insert(0, 'label', new_train_labels)
        test_data.insert(0, 'label', new_test_labels)
        log_file_name = 'results_full_data_translated_old_vs_new_mpnet_RILE_only.log' # if sbert else 'results_full_data_translated_old_vs_new_roberta_RILE_only.log'
    else:
        log_file_name = 'results_full_data_translated_old_vs_new_mpnet.log' # if sbert else 'results_full_data_translated_old_vs_new_roberta.log'
    train_labels = set(train_data.label.unique())

    if not test_run:
        print(f'Writing to {log_file_name}')
        with open(log_file_name, 'a') as out:
            print('Test years: 2019–2021', file=out)
            print(f'{train_data.shape=}', file=out)
            print(f'{test_data.shape=}', file=out)
    print('Test years: 2019–2021')
    ...
    classification_head = ClassificationHead(
        input_dim=(768 if sbert else 1024),
        out_dim=n_classes)
    ...
    for epoch_n in range(n_epochs):
        ...
        epoch_test_accuracy, epoch_test_predictions = validate_epoch(
            ...
            classification_head,
            sbert=sbert)
        print(f'2019–2021, {epoch_n+1}, {epoch_train_loss=}, {epoch_dev_accuracy=}, {epoch_test_accuracy=}')
        if not test_run:
            with open(log_file_name, 'a') as out:
                print(f'2019–2021, {epoch_n+1}, {epoch_train_loss=}, {epoch_dev_accuracy=}, {epoch_test_accuracy=}', file=out)
        ...
        if not test_run:
            with open(log_file_name, 'a') as out:
                print(
                    ...
                    file=out)
        if epoch_n > 0:
            with open(log_file_name.replace('.log', f'_{epoch_n+1}.json'), 'w') as out:
                json.dump(rile_by_manifesto, out, indent=2)
            with open(log_file_name.replace('.log', f'_predictions_{epoch_n+1}.json'), 'w') as out:
