...
def main():
    ...
    for epoch_n in tqdm(range(n_epochs), desc='Epochs', leave=False):
        ...
        if epoch_dev_correlation > best_dev_correlation:
            torch.save(encoder.state_dict(), 'encoder_checkpoint_longformer_ovn.pt')
            torch.save(regression_head.state_dict(),
                       'regression_head_checkpoint_longformer_ovn.pt')
    ...
    results_dict = {
        'dev': {
            'manifesto_ids': data_dev.manifesto_id.to_list(),
            'predictions': {}
        },
        'test': {
    ...
    print(
        f'Test Spearman correlation after {n_epochs} epochs: {test_correlation}')
    for epoch_n in range(n_epochs):
        results_dict['dev']['predictions'][epoch_n] = dev_predictions[epoch_n]
    with open('results_longformer_ovn.json', 'w') as out:
